...
Protocol (minimal, msgpack over websockets — no external `openpi` dependency):
  ...
  - "direction" selects which trained text caption to condition on - the
    ...
    disambiguating signal (see _DIRECTION_CAPTIONS / action_policy_roboracer_edge.py);
...
# Must be set before the first CUDA allocation. PyTorch's caching allocator
# never returns freed blocks to the driver, so a process's CUDA footprint is
# its high-water mark; expandable_segments lets it grow segments in place
# rather than creating a fresh block per size class, which sharply narrows
# the reserved-vs-allocated gap (Nano server: 9.20 GB live vs 37.59 reserved).
...
from cosmos_framework.data.generator.action.datasets.roboracer_dataset import RoboracerDataset, _STATS_PATH
...
# --task-description for orin10 (left) / orin13 (right) — see action_policy_roboracer_edge.py.
...
def _log_cuda_mem(tag: str) -> None:
    """Log real (allocated) vs bookkeeping (reserved) CUDA memory.

    `allocated` is what live tensors hold -- the number that decides whether
    this fits the car's 8 GB budget. `reserved` is what PyTorch has taken
    from the driver and kept in its pool; it is what nvidia-smi/nvtop show,
    and it is normally much larger. Do not size hardware off `reserved`.
    """
    if not torch.cuda.is_available():
        return
    log.info(
        f"[roboracer-policy-server] CUDA {tag}: "
...
class RoboracerPolicyService:
    """Loads the checkpoint once; runs mode="wam" (past-only, deployable)
    ...
    def __init__(self, checkpoint_path: str, eval_root: str, vae_path: str) -> None:
        ...
        # Release the model-loading temporaries (safetensors staging, bf16
        # originals freed after 4-bit quantization). They are never reused
        # while serving, and holding them inflates the reserved figure that
        # nvidia-smi reports by tens of GB.
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.reset_peak_memory_stats()
        _log_cuda_mem("after load")
        self._logged_peak = False
    ...
    def predict(self, image_hwc_uint8: np.ndarray, direction: str = _DEFAULT_DIRECTION,
        ...
        with self._lock:
            ...
            sequence_plan = build_sequence_plan_from_mode(
                mode="wam", video_length=video.shape[1], action_length=action_zeros.shape[0]
            )
            data_batch = {
                "video": [[video]],
                "action": [[action_zeros]],
            ...
            if not self._logged_peak:
                _log_cuda_mem("after first request")
                self._logged_peak = True
            return {
...
def main(
    ...
    vae_path: str = "/scratch/tarunrav/cosmos-framework/examples/checkpoints/wan22_vae/Wan2.2_VAE_fp16.pth",
) -> None:
