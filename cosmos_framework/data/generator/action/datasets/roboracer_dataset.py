...
_STATS_PATH = Path(__file__).parent.parent / "normalizer_stats" / "roboracer_stats.json"
...
class RoboracerDataset(ActionBaseDataset):
    # Declared so the Cosmos3-Edge exporter can populate checkpoint.json's policy
    # manifest (action_chunk_size / conditioning_fps / domain_name). Mirrors the
    # domain_name="av" this dataset passes when building its ActionSpec.
    EMBODIMENT_TYPE = "av"
    ...
    def __init__(
        ...
        # policy per sample, base_dataset.py _MODE_CHOICES) and mode="wam"
...
def get_action_roboracer_sft_dataset(
    *,
    ...
    fps: float = 15.0,
    chunk_length: int = 32,
    mode: str = "wam",  # safe/deployable default — see RoboracerDataset.__init__
...
# The roboracer experiment configs use the factory below as their `_target_`,
# so the exporter looks for EMBODIMENT_TYPE here rather than on the class.
get_action_roboracer_sft_dataset.EMBODIMENT_TYPE = "av"
